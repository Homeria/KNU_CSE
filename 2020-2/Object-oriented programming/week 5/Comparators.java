...
public class Comparators {
    ...
    // Comparator의 static 메소드를 호출하는 문장을 넣어준다. Comparator.naturalOrder()
    ...
    // Hint:
    // comparing과 thenComparing을 연이어 적는다(chaining).
    // comparing의 인자로는 Person의 lastName을 얻는 메소드를 메소드 레퍼런스 형태로 준다. Person::lastName
    // thenComparing의 인자로는 Person의 firstName을 얻는 메소드를 메소드 레퍼런스 형태로 준다. Person::firstName
    ...
    public void comparator07() {
        Comparator<Person> comparebyAge = Comparator.comparingInt(p -> p.getAge());
    ...
    // Hint:
    // Integer는 Comparable입니다. 
    // (API 문서에서 Integer를 찾아보면 알 수 있습니다! 맨 윗 부분에 적혀 있습니다. 찾아 보세요.)
    // 따라서 compareTo 메소드를 갖고 있습니다.
    // 이 메소드는 Integer 값의 크기에 따라 음수, 0, 양수를 반환합니다.
    // 이 메소드를 가리키는 메소드 레퍼런스를 적으면 됩니다.
    //
    // 이 메소드를 Integer::compare하고 적으면 Class::instanceMethod 행식입니다.
    // IntBinaryOperator에게 메소드를 호출할 때 주는 두 개의 인자 중
    // 첫 번째 것이 이 메소드 호출의 receiver가 되고 두 번째 것은 compare 메소드 인자로 넘어갑니다.
    // 그러니까 (x, y) -> x.compare(y)로 적은 람다식과 같습니다.
