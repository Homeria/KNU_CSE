...
public class DivideAndConquer implements Runnable  {
	...
	// 전체 배열
	final static int NUM_OF_VALUES =  cores * 33000000;
	...
	public static void main(String args[]) {
		 ...
		 ************************************/
		{
			...
			for( int i=0; i<cores; i++ ) {
				...
				thread[i] = new Thread(new DivideAndConquer(i));
				thread[i].start();
		 ...
		 ************************************/
		{
			System.out.println("스트림으로 계산.");
			int total = 0;
			long startTime = System.currentTimeMillis();
			
			IntStream stream = Arrays.stream(values);
			// 누적값 a에 스트림 원소 b를 더한 값을 반환한다.
			// 반환된 값이 새로운 누적값 a가 된다.
			// 누적값의 초기값은 0이다.
			total = stream.reduce(0, (a, b) -> a + b);
		 ...
		 ************************************/
		{
			...
			// 반환된 값이 새로운 누적값 a가 된다.
			// 누적값의 초기값은 0이다.
			total = stream.parallel().reduce(0, (a, b) -> a + b);
