...
public class Game 
{
    ...
    private void createRooms()
    {
        ...
        dongBang.addItem(new Item("portion", "먹으면 체력이 5만큼 증가하는 건강음료", 5));
    ...
    private void back(Command command) {
    	if (!command.hasSecondWord()) {
    		...
    		System.out.println("한 단계만 뒤로만 갈 수 있습니다.");
    ...
    private void take(Command command) {
    	if (!command.hasSecondWord()) {
    		...
    		Item item = player.takeItem(secondWord);
    		if (item == null) {
    ...
    private void drop(Command command) {
    	if (!command.hasSecondWord()) {
    		System.out.println("drop 명령어는 인자가 필요합니다.");
    	} else {
    		String secondWord = command.getSecondWord();
    		Item item = player.dropItem(secondWord);
    		if (item == null) {
    			System.out.println("아이템을 버릴 수 없습니다.");
    		} else {
    			List<Item> items = player.getItems();
    			printItems(items);
    		}
