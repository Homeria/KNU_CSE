...
public class NewNewCrypt {
				...
				if ((dicWord.length() == lineWord.length()) && (findMaxCount(dicWord) == findMaxCount(lineWord))) {
					...
					for(int p = 0; p < dicWord.length(); p++) {
						for(int q = 0; q < lineWord.length(); q++) {
							...
							if(!map.containsKey(dicChar) || !map.containsValue(dicChar)) {
	...
	private static void printMatrixList(List<ArrayList<String>> list) {
		
		System.out.println("Yes=================================================================");
		
		System.out.println();
		for(int i = 0; i < list.size(); i++) {
			ArrayList<String> target = list.get(i);
			System.out.print(i + " :");
	...
	private static String getMostSimilar(String dict, List<String> list) {
		...
		for(int i = 0; i < list.size(); i++) {
			...
			String s = list.get(i);
			int size = s.length();
			ArrayList<String> tmp = matrix.get(size - 1);
			tmp.add(s);
			matrix.set(size - 1, tmp);
		}
		
		printMatrixList(matrix);
		
		ArrayList<String> sample = matrix.get(dict.length() - 1);
