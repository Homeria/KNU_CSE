				...
				if (sameLength.size() == 1) {
					...
					if (okay.size() == 1) {
						...
						for(int j = 0; j < sameLength.size(); j++) {
							String lengthS = sameLength.get(j);
							System.out.println("lengthS = " + lengthS);
							if (isOkay(dictS, lengthS) && (getDuplicate(dictS) == getDuplicate(lengthS))) {
								okay2.add(lengthS);
							}
						}
						System.out.println("okay2 = " + okay2);
						if (okay.size() == 1) {
							wordMap.put(dictS, okay2.get(0));
							removeTarget(dictS, dictList);
							removeTarget(okay2.get(0), lineList);
							additionalMapping2(list, dictS, okay2.get(0));
						} else {
