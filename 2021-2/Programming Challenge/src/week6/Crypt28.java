			...
			for(int i = 0; i < dictTmp.size(); i++) {
				...
				if (sameLength.size() == 1) {
					...
					for(int j = 0; j < sameLength.size(); j++) {
						...
						System.out.println("before getCount : dictS = " + dictS + ", lengthS = " + lengthS);
						if(getCount(dictS, lengthS) >= 2) {
							test.add(lengthS);
							System.out.println("getCount : dictS = " + dictS + ", lengthS = " + lengthS);
//							wordMap.put(dictS, lengthS);
//							removeTarget(dictS, dictList);
//							removeTarget(sameLength.get(j), lineList);
//							additionalMapping2(list, dictS, sameLength.get(j));
						}
